...
public interface FeedDetailsDao {
    ...
    @Query("SELECT id, title, name, description, imageUrl, text, time FROM feed_details")
    List<FeedDetails> getFeedDetailsList();

    @Query("SELECT id, title, name, description, imageUrl, text, time FROM feed_details WHERE id = :id")
